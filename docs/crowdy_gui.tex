...
we are building the user interface on top of exposes a RESTful API.  To
retrieve data, our system makes an HTTP GET request to the server.  The server
returns data in the JSON format, which is easy to decode in Javascript.

We are writing the portion of the web application that runs on the server in
...
write testable code.  We are using the Jinja templating engine to generate the
HTML pages.
